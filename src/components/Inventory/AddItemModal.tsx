...
interface AddItemModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAddItem: (item: Omit<InventoryItem, 'id' | 'createdAt' | 'lastUpdated'>) => void;
}
...
export const AddItemModal: React.FC<AddItemModalProps> = ({
  ...
  const validateForm = () => {
    ...
    if (formData.currentStock < 0) newErrors.currentStock = 'Valid current stock is required';
    if (formData.minStock < 0) newErrors.minStock = 'Valid minimum stock is required';
    if (formData.maxStock < 0) newErrors.maxStock = 'Valid maximum stock is required';
    if (formData.unitPrice < 0) newErrors.unitPrice = 'Valid unit price is required';
        ...
        <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between rounded-t-xl">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">Add a new item to your clinic inventory</p>
            </div>
          </div>
          <button
            onClick={onClose}
